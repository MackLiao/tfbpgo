...
|---|------------------------------|-----------------------------|---------------|------|----------|
| 1 | Dataset list grouped by Binding / Perturbation, sorted by display name | `server/sidebar.py:109-132` | Present (grouped, sort comes from backend ORDER BY db_name not display_name) | Sort is by `db_name` in `datasets.go:36-37`, Shiny sorts by `display_name` (which begins with year) | P1 |
...
| 3 | Default-active dataset set (`harbison, rossi, chec_m2025, hackett, callingcards, kemmeren, degron` on first load) | `utils/vdb_init.py:40-50`, sidebar.py:149-152, 158-170 | Missing — `?binding=`/`?perturbation=` start empty | First-time visitors see empty matrix / no preselection | P1 |
| 4 | Default filter set (`harbison.condition=YPD`, `rossi.treatment=Normal`, `chec_m2025.Experimental condition=standard`, `hackett.time=[45,45]`) | `utils/vdb_init.py:55-68` | Missing — `?filters=` starts empty | Out-of-the-box query parity broken for downstream tabs | P0 |
...
| 9 | Filter modal: boolean fields → switch with `"{field} ({description})"` label | `ui.py:104-124` | Missing | No column descriptions in backend manifest | P0 |
...
| 12 | Per-field "Apply to all datasets" toggle in modal header (common fields only) | `ui.py:63-69, 191-197`, sidebar.py:541-548, 613-650 | Missing | — | P0 |
| 13 | Regulator card inside modal: searchable multi-select of `SYMBOL (LOCUS_TAG)` | `ui.py:329-417`, dataset_row.py:192-205 | Missing — `RegulatorPicker.tsx` exists but is single-select, only called from Binding/Perturbation pages | Need multi-select w/ `remove_button` and display-label map | P0 |
| 14 | Regulator card: "Apply to all datasets" + "Clear" button | `ui.py:393-405`, sidebar.py:443-469 | Missing | — | P1 |
| 15 | Pairwise regulator filter context in modal (note + restricted choices when `from_pair` set) | `ui.py:339-380` | Missing | Depends on matrix click-through | P1 |
| 16 | Modal footer: "Reset" + "Apply Filters" buttons | `ui.py:478-489`, sidebar.py:401-441, 471-677 | Missing | — | P0 |
| 17 | Reset semantics: clears this dataset's filters AND common-field filters from all datasets | `sidebar.py:401-441` | Missing | — | P1 |
| 18 | Apply semantics: propagates common-field filters to all datasets per `apply_to_all`; auto-activates dataset | `sidebar.py:471-677` | Missing | — | P0 |
...
| 20 | Pending-vs-committed staging: changes to toggles + filters do not fire downstream queries until user clicks sidebar "Apply" | `sidebar.py:154-189, 679-691` | Missing — every checkbox toggle navigates URL immediately | Live re-query on every checkbox toggle = exact problem the rewrite was meant to fix | P1 |
| 21 | Sidebar "Apply" button visible only when pending differs from committed | `sidebar.py:906-911` | Missing | — | P2 |
| 22 | Dataset description shown as tooltip on each row label | `dataset_row.py:50-55` | Missing — description IS in `/datasets` payload but not rendered as tooltip (shown as raw `<dt>/<dd>` block) | Use as hover tooltip per Shiny UX | P2 |
| 23 | Sidebar collapse / expand toggle (header chevron) | `server/sidebar.py:218-229, 873-902` | Missing | — | P2 |
| 24 | Search box in sidebar filtering datasets by display-name substring | `server/sidebar.py:815-820, 839-863` | Missing | — | P2 |
...
| 28 | Diagonal-cell click → modal explaining sample/regulator multiplicity, listing differentiating columns | `workspace.py:127-185`, queries.py:133-185, ui.py:509-544 | Missing | No `/regulator-breakdown` endpoint | P1 |
...
| 30 | Applying common-regulators from a pair writes `regulator_locus_tag` filter to ALL datasets with `from_pair` annotation and highlights cell | `workspace.py:231-290` | Missing | — | P1 |
| 31 | Re-clicking the active off-diagonal pair clears the regulator filter and unhighlights | `workspace.py:192-218, 333-353` | Missing | — | P2 |
...
| 34 | "No datasets match your search" empty state | `sidebar.py:865-871` | Missing | — | P2 |
| 35 | Per-dataset CSV+README export bundled as `.tar.gz` (one dir per dataset, `metadata.csv` + `annotated_features.csv` + `README.md`), with progress bar | `export.py:39-213`, sidebar.py:693-780 | Missing | No backend export endpoint; would need streamed CSV + tar build | P1 |
| 36 | Sidebar footer with "Export Selected Datasets" button — visible only when ≥1 dataset is active | `sidebar.py:916-933` | Missing | — | P1 |
