...
| 10 | Regulator selectize dropdown labelled `"Regulator"`, choices = union of regulators present in any pair's correlation data, displayed by gene symbol with fallback to locus tag, sorted case-insensitively | `workspace.py:366-407` (`regulator_selector` — `ui.input_selectize` with `choices = {locus: sym}` sorted by symbol lower). Selected value persists across re-renders when still present. | PARTIAL | React `RegulatorPicker` (`components/RegulatorPicker.tsx:1-45`) is a typeahead search hitting `/regulators?search=`, NOT a selectize bound to the active dataset intersection. So a user can type and pick *any* regulator from the global table — including one that has zero overlap with the active pairs. There is no narrowing to "regulators present in this set of pairs". | P1 |
...
| 42 | Tooltip on selected overlay dots: `"<gene symbol>\nr = X.XXX\n<dataset A>: <cond A>\n<dataset B>: <cond B>"` (HTML-escaped, joined with `<br>`) | `workspace.py:295-306` | MISSING | No overlay exists. | P0 |
