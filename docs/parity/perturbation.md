...
| 10 | Selected-regulator hover with HTML-escaped condition labels per dataset: `display`, `r=…`, `"{label_a}: {cond}"`, `"{label_b}: {cond}"` | `server/workspace.py:276-287` | DONE | Closed in Phase C — commit `3e4b80a`. `PerturbationCorrBoxplot` overlay hovertext is multi-line `symbol / r / displayA: condA / displayB: condB`; every DB-sourced string passes through `lib/html-escape.ts`. Condition map sourced from `/datasets/{db}/sample-conditions`. | — |
...
| 13 | Regulator drop-down (`input_selectize`), choices = union of regulators present in any pair's correlation data, labelled by gene symbol, sorted case-insensitively | `server/workspace.py:347-388` | DONE | Closed in Phase C — commit `3e4b80a`. `ActivePairRegulatorPicker` narrows the regulator selectize to regulators present in the `/perturbation/correlations` response (native `<select>` under 50 options, typeahead above). Falls back to global RegulatorPicker before corrQuery resolves. | — |
...
| 26 | Condition labels source: `{db}_meta` table joined on `sample_id` via `AppDatasets.condition_cols` per dataset; `" / "` separator from `build_condition_label` | `utils/sample_conditions.py:25-94`, `server/workspace.py:99-123` | DONE | Closed in Phase C — commit `3e4b80a`. New `GET /api/v/{v}/datasets/{db}/sample-conditions` returns `{sample_id: condition_label}` built from `dataset_manifest.condition_cols` (schema v4); `" / "` separator preserved. | — |
