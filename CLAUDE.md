...
**Status as of 2026-05-12:** Phase 1 (Go backend) complete on branch `phase-1-go-backend`. The backend embeds the `data_prep` artifact contract (`schema_version 2`, `MinSchemaVersion=MaxSchemaVersion=2` in `backend/internal/db/startup.go`). All endpoints implemented (`/healthz`, `/readyz`, `/metrics`, `/api/version`, `/api/v/{v}/datasets|regulators|binding|perturbation|comparison/topn|comparison/dto`, plus the `/_ref` HTML view for parity diffing). Per-endpoint parity tests scaffolded in `tests/parity/` but each `t.Skip`s pending Python-side fixture recording (manual step via `make parity-record`). DuckDB pool opens read-only with §6.3 settings; cache stack is ristretto+singleflight+`Wait()` per §8.1; identifier whitelisting backed by `dataset_manifest`/`field_manifest`. **API contract is frozen.** Phase 2 (React) and Phase 3 (Docker + Traefik) not yet started. The spec at `docs/superpowers/specs/2026-05-12-go-react-rewrite-design.md` remains the source of truth for architecture decisions. Per-phase plans live under `docs/superpowers/plans/`.
