...
**Status as of 2026-06-10:** Phases 0, 1, 2, and 3 are complete and merged to main. Phase 0 (`data_prep/`, Python, **`schema_version=6`**) ships the runtime artifact + test fixture (49 unit tests). The Go binary's compatible schema range is **[6, 6]** (`backend/internal/db/startup.go`) — a v5-or-earlier artifact is rejected at startup, so rollback targets must also be v6. `schema_version=5` (2026-05-28 parity re-audit) added `dataset_manifest.upstream_cols`/`description`, populated `field_manifest.description`/`level_definitions`, restricted the materialized `hackett`/`hackett_meta` tables to the analysis set at build time, and rebuilt the fixture via the real manifest builders. `schema_version=6` (2026-06-11 promoter-sets + comparison parity re-audit) added `dataset_manifest.is_primary` (used for the selector `is_primary` hide — only base datasets are shown; the 11 promoter-set variants are comparison-only) plus `log10p_col`/`neglog10p_col` (used by the log10pval scatter transform). `schema_version=3` externalized the previously hard-coded perturbation `effect_col` / `pvalue_col` and the field `role` into `dataset_manifest` / `field_manifest`; the Go service reads them at startup via `db.NewWhitelist` and `comparison_topn.buildResponsiveExpr`. `schema_version=4` adds per-dataset and per-field UX metadata to those same manifests: `dataset_manifest` gains `default_active` / `default_filters` / `condition_cols` (mirroring Shiny's `DEFAULT_ACTIVE_DATASETS`, `DEFAULT_DATASET_FILTERS`, and the runtime-computed `AppDatasets.condition_cols`); `field_manifest` gains `description` / `level_definitions` / `ui_kind_override` / `numeric_level_sort` (the last two mirror Shiny's `FIELD_TYPE_OVERRIDES`, replacing the Go-side `fieldTypeOverrides` constant). The `/datasets` and `/datasets/{db}/fields` JSON contract extends in lockstep. Phase 1 (`backend/`, Go) ships the full API surface (42 unit tests + parity scaffolding). Phase 2 ships the React SPA (`frontend/`, Vite + React 18 + TanStack Query + Zustand + Plotly custom bundle), the OpenAPI doc generated from handler annotations, the `GET /api/v/{v}/regulators/resolve` endpoint, the embedded `backend/static/dist/` bundle served by the Go binary, `Cache-Control: public, max-age=31536000, immutable` headers on `/api/v/*` responses, and the parity snapshot scaffold under `tests/parity/`. Phase 3 delivers a multi-stage `Dockerfile` (Node → Go-with-CGO → Debian-slim runtime), `docker-compose.yml` with Traefik routing for both the new Go service on `tfbindingandperturbation.com` and the legacy Shiny on `legacy.tfbindingandperturbation.com`, an S3 artifact publish path (`deploy/s3-upload.sh` + `.github/workflows/artifact-publish.yml`) plus a one-shot init container that downloads + sha256-verifies into a named volume, k6 acceptance scripts (`profile.js` warm-cache, `cold_burst.js` singleflight gate), the widened test fixture so all 11 parity URLs pass, the now-observed `db_pool_wait_duration_seconds` metric, the operator runbook at `deploy/README.md`, and the cutover summary template at `tests/loadtest-summary.md`. All API endpoints remain in place: `/healthz`, `/readyz`, `/metrics`, `/api/version`, plus 17 versioned routes under `/api/v/{v}/`: `datasets`, `datasets/{db}/fields`, `datasets/{db}/regulators`, `datasets/{db}/sample-conditions`, `selection/matrix`, `selection/breakdown`, `regulators`, `regulators/resolve`, `binding`, `binding/corr`, `binding/scatter`, `perturbation`, `perturbation/correlations`, `perturbation/scatter`, `comparison/topn`, `comparison/dto`, `export`. DuckDB pool opens read-only with §6.3 settings; cache is ristretto+singleflight+`Wait()` per §8.1 with `OnEvict` wired; identifier whitelisting backed by `dataset_manifest`/`field_manifest` with defense-in-depth regex re-verification at startup; DoS caps on `?filters=`, `?top_n=`, dataset CSV lists, `?search=`. **API contract is extended (regulators/resolve added) and frozen — no further breaking changes without a version bump.** **The cutover gate run (warm + cold-burst k6, peak-RSS, observability checks) is an operational step** performed on the deploy host (requires Docker + k6); the committed `tests/loadtest-summary.md` is a TEMPLATE that the operator fills in with real numbers and commits alongside the cutover deploy. The spec at `docs/superpowers/specs/2026-05-12-go-react-rewrite-design.md` remains the source of truth. Per-phase plans live under `docs/superpowers/plans/`. **Next:** operator runs the cutover gate per `deploy/README.md`, records numbers in `tests/loadtest-summary.md`, then cuts DNS to the Go service.

## Outstanding follow-ups (post-cutover)

All four Phase 2 deferrals have been absorbed into Phase 3 (fixture widening — parity 11/11 green; `db_pool_wait_duration_seconds` observed via `samplePoolStats`; `backend-build-only` Makefile target; Plotly overage to be decided at cutover load test rather than pre-emptively dropping a trace). The Phase 1.6 follow-up to move the hard-coded measurement column maps into `dataset_manifest` shipped as `schema_version=3` (commit `1976f59`); per-dataset/per-field UX metadata followed as `schema_version=4` (Phase C task C1).
...
```
backend/    # Go service (chi, marcboeker/go-duckdb/v2, sqlx, ristretto, singleflight)
...
The highest-risk part is **query parity** between Go + marcboeker/go-duckdb/v2 and the current Python DuckDB. Do not start the React frontend in parallel with backend query work.
