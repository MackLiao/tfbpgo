// Mirrors plots/BindingScatterPair.tsx 1:1 with /perturbation/scatter.
// Keep these in sync.
import type { Schemas } from "@/api/client";
import { htmlEscape } from "@/lib/html-escape";
import { PlotLazy } from "./PlotLazy";

// Per-pair scatter for the Perturbation module.
//
// Mirrors reference/tfbpshiny/modules/perturbation/server/workspace.py:471-581
...
// PerturbationScatterRow. When the regulator yielded zero rows the slot
// collapses to a zero-width span (Shiny ui.span()).

export interface PerturbationScatterPairProps {
  resp: Schemas["ScatterResponse"];
  displayNameA: string;
  displayNameB: string;
}

export function PerturbationScatterPair({
  resp,
  displayNameA,
  displayNameB,
}: PerturbationScatterPairProps) {
  // Collapse empty pairs — Shiny renders an empty span; the scatter row surfaces
  // "regulator not found" notices separately, so silence here is fine.
